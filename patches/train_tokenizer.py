...
def init_csv_log(save_dir: str, resume: bool) -> str:
    ...
    if not resume or not os.path.isfile(csv_path):
        with open(csv_path, 'w', newline='') as f:
            csv.writer(f).writerow(['type', 'epoch', 'step', 'loss', 'lr'])
    return csv_path
...
def train_model(model, device, config, save_dir, logger, rank, world_size):
    ...
    # Resume: detect how many epochs already completed by checking checkpoint existence
    start_epoch = 0
    resume_ckpt = os.path.join(save_dir, 'checkpoints', 'best_model')
    if os.path.isdir(resume_ckpt):
        ...
        if os.path.isfile(summary_path):
            ...
            with open(summary_path) as _f:
                _s = _json.load(_f)
            # If final_result exists, all epochs were completed
            if 'final_result' in _s:
                start_epoch = config['epochs']
            else:
                start_epoch = _s.get('completed_epochs', 1)
        else:
            start_epoch = 1
        if rank == 0:
            if start_epoch >= config['epochs']:
                print(f"[Resume] Tokenizer already fully trained ({config['epochs']} epochs). Skipping all training.")
            else:
                print(f"[Resume] Skipping epoch(s) 1–{start_epoch}, resuming from Epoch {start_epoch + 1}.")

    # 初始化 CSV（確認 start_epoch 後才建立）
    if rank == 0:
    ...
    for epoch_idx in range(start_epoch, config['epochs']):
        ...
        for i, (ori_batch_x, _) in enumerate(train_loader):
            ...
            if rank == 0 and (batch_idx_global_train + 1) % config['log_interval'] == 0:
                ...
                if csv_path:
                    append_csv(csv_path, ['train', epoch_idx + 1, batch_idx_global_train + 1,
                                          round(avg_loss, 6), round(lr, 8)])
        ...
        if rank == 0:
            ...
            if avg_val_loss < best_val_loss:
                ...
                if logger:
                    logger.log_model("best_model", best_save_path)

            # CSV: 記錄 epoch val loss
            if csv_path:
                append_csv(csv_path, ['val', epoch_idx + 1, -1, round(avg_val_loss, 6), ''])

            # Write completed epoch count so resume can detect progress
            summary_path = os.path.join(save_dir, 'summary.json')
            _summary_data = {}
            if os.path.isfile(summary_path):
                import json as _json
            ...
            with open(summary_path, 'w') as _f:
                ...
                _json2.dump(_summary_data, _f, indent=4)
...
def main(config: dict):
    ...
    # Model Initialization — resume from local checkpoint if available
    resume_checkpoint = os.path.join(save_dir, 'checkpoints', 'best_model')
    if os.path.isdir(resume_checkpoint):
        if rank == 0:
            print(f"[Resume] Found local checkpoint at {resume_checkpoint}, loading instead of HuggingFace weights.")
        model = KronosTokenizer.from_pretrained(resume_checkpoint)
    else:
        if rank == 0:
            print(f"[Init] No local checkpoint found, loading from {config['pretrained_tokenizer_path']}.")
