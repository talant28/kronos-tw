...
def setup_ddp():
    ...
    import sys as _sys
    backend = "gloo" if _sys.platform == "win32" else "nccl"
    ...
    if world_size == 1:
        # Single GPU: use FileStore to avoid TCPStore/libuv dependency
        import tempfile
        store_path = os.path.join(tempfile.gettempdir(), "kronos_dist_store")
        ...
        print(f"[DDP Setup] Single-GPU mode, backend={backend}, FileStore")
    else:
        dist.init_process_group(backend=backend)
        print(f"[DDP Setup] Multi-GPU mode, backend={backend}")
